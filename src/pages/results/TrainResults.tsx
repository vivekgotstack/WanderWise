import { useEffect, useState } from "react";
import { useSearchParams, useNavigate } from "react-router-dom";
import axios from "axios";
import { useTheme } from "@/contexts/ThemeContext";

type Train = {
  id: number;
  trainName: string;
  departureTime: string;
  arrivalTime: string;
  type: string;
  price: number;
};

export default function TrainResults() {
  const [searchParams] = useSearchParams();
  const navigate = useNavigate();
  const { theme } = useTheme();

  const [trains, setTrains] = useState<Train[]>([]);
  const [loading, setLoading] = useState(true);

  const [sortBy, setSortBy] = useState("price");
  const [direction, setDirection] = useState("asc");

  const source = searchParams.get("source") || "";
  const destination = searchParams.get("destination") || "";
  const date = searchParams.get("date") || "";

  const isDark = theme !== "light";

  // 🔥 LOGO MAP
  const logoMap: any = {
    "Rajdhani Express": "/trains/rajdhani.png",
    "Shatabdi Express": "/trains/shatabdi.png",
    "Duronto Express": "/trains/duronto.png",
    "Vande Bharat": "/trains/vande.png",
    "Garib Rath": "/trains/garib.png",
  };

  // 🔥 RANDOM STOP TAGS
  const stopTags = ["Direct", "1 Stop", "2 Stops"];

  const getRandomStops = () =>
    stopTags[Math.floor(Math.random() * stopTags.length)];

  // 🔥 FIXED DURATION (NO NEGATIVE)
  const formatDuration = (dep: string, arr: string) => {
    const d = new Date(`1970-01-01T${dep}`);
    let a = new Date(`1970-01-01T${arr}`);

    // 🚀 if arrival < departure → next day
    if (a.getTime() < d.getTime()) {
      a.setDate(a.getDate() + 1);
    }

    const diff = a.getTime() - d.getTime();
    const mins = Math.floor(diff / 60000);
    const h = Math.floor(mins / 60);
    const m = mins % 60;

    return `${h}h ${m}m`;
  };

  useEffect(() => {
    const fetchTrains = async () => {
      try {
        const res = await axios.get(
        `${import.meta.env.VITE_API_BASE_URL}/trains/search`,
          {
            params: { source, destination, date },
          }
        );

        let data: Train[] = res.data.data;

        // 🔥 ADD EXTRA OPTIONS
        const extra: Train[] = [
          {
            id: 101,
            trainName: "Vande Bharat",
            departureTime: "06:00",
            arrivalTime: "10:30",
            type: "AC Chair Car",
            price: 1450,
          },
          {
            id: 102,
            trainName: "Rajdhani Express",
            departureTime: "20:00",
            arrivalTime: "06:30",
            type: "AC 2 Tier",
            price: 2200,
          },
          {
            id: 103,
            trainName: "Duronto Express",
            departureTime: "22:15",
            arrivalTime: "07:00",
            type: "Sleeper",
            price: 980,
          },
        ];

        data = [...data, ...extra];

        // 🔥 SORT
        data = [...data].sort((a, b) => {
          let valA: any = a[sortBy as keyof Train];
          let valB: any = b[sortBy as keyof Train];

          if (sortBy === "departureTime") {
            valA = new Date(`1970-01-01T${valA}`);
            valB = new Date(`1970-01-01T${valB}`);
          }

          return direction === "asc"
            ? valA > valB ? 1 : -1
            : valA < valB ? 1 : -1;
        });

        setTrains(data);
      } catch (e) {
        console.error(e);
      } finally {
        setLoading(false);
      }
    };

    fetchTrains();
  }, [source, destination, date, sortBy, direction]);

  return (
    <div
      className="min-h-screen text-center px-4 sm:px-6 md:px-12 lg:px-20 pt-80 sm:pt-60 md:pt-36 lg:pt-40 pb-20 flex flex-col items-center"
      style={{
        background: isDark
          ? "#0b0c32"
          : "radial-gradient(125% 125% at 50% 90%, #fff 40%, #6366f1 100%)",
      }}
    >
      {/* HEADER */}
      <div className="w-full max-w-6xl mb-6">
        <h1
          className={`text-xl sm:text-2xl md:text-3xl font-bold ${isDark ? "text-gray-200" : "text-gray-800"
            }`}
        >
          Trains from {source} → {destination}
        </h1>
      </div>

      {/* LOADING */}
      {loading && (
        <div className="mt-20 flex flex-col items-center gap-3">
          <div className="w-10 h-10 border-4 border-indigo-500 border-t-transparent rounded-full animate-spin"></div>
          <p className="text-gray-400">Fetching trains...</p>
        </div>
      )}

      {/* EMPTY */}
      {!loading && trains.length === 0 && (
        <div className="mt-24 text-center max-w-md">
          <div className="text-5xl mb-4">🚆</div>

          <h2
            className={`text-xl font-semibold ${isDark ? "text-gray-300" : "text-gray-700"
              }`}
          >
            No trains found
          </h2>

          <p className="mt-2 text-gray-500 text-sm">
            No trains available for this route.
          </p>

          <button
            onClick={() => navigate("/trains")}
            className="mt-6 px-6 py-2 bg-indigo-600 text-white rounded-lg hover:bg-indigo-700 transition"
          >
            Try Another Search
          </button>
        </div>
      )}

      {/* CONTENT */}
      {!loading && trains.length > 0 && (
        <>
          {/* SORT */}
          <div className="flex flex-wrap gap-3 mb-6 justify-center">
            <select
              value={sortBy}
              onChange={(e) => setSortBy(e.target.value)}
              className={`p-2 rounded-lg border ${isDark
                  ? "bg-[#1a1c4b] text-white border-gray-600"
                  : "bg-white border-gray-300"
                }`}
            >
              <option value="price">Price</option>
              <option value="departureTime">Departure</option>
            </select>

            <select
              value={direction}
              onChange={(e) => setDirection(e.target.value)}
              className={`p-2 rounded-lg border ${isDark
                  ? "bg-[#1a1c4b] text-white border-gray-600"
                  : "bg-white border-gray-300"
                }`}
            >
              <option value="asc">Asc</option>
              <option value="desc">Desc</option>
            </select>
          </div>

          {/* RESULTS */}
          <div className="grid gap-5 w-full max-w-6xl">
            {trains.map((train) => {
              const stops = getRandomStops();

              return (
                <div
                  key={train.id}
                  className={`p-5 rounded-xl shadow-md hover:shadow-xl transition ${isDark
                      ? "bg-[#15173a] text-gray-200"
                      : "bg-gradient-to-br from-pink-50 via-white to-indigo-50 border border-indigo-100"
                    }`}
                >
                  <div className="flex justify-between items-center flex-wrap gap-4">

                    {/* LEFT */}
                    <div className="flex items-center gap-3">
                      <img
                        src={logoMap[train.trainName]}
                        className="w-12 h-12 object-contain"
                        onError={(e: any) => (e.target.style.display = "none")}
                      />

                      <div>
                        <h2 className="font-semibold">
                          {train.trainName}
                        </h2>
                        <p className="text-sm text-gray-400">
                          {source} → {destination}
                        </p>
                      </div>
                    </div>

                    {/* PRICE */}
                    <div className="text-right">
                      <p className="text-lg font-bold text-indigo-500">
                        ₹{train.price}
                      </p>
                      <p className="text-xs text-gray-400">
                        {train.type}
                      </p>
                    </div>
                  </div>

                  <div className="mt-4 flex justify-between items-center flex-wrap gap-4">
                    <div>
                      <p className="text-sm text-gray-400">
                        {train.departureTime} → {train.arrivalTime}
                      </p>

                      <p className="text-xs text-gray-400">
                        {formatDuration(
                          train.departureTime,
                          train.arrivalTime
                        )} • {stops}
                      </p>
                    </div>

                    <button className="px-4 py-2 bg-indigo-600 text-white rounded-lg hover:bg-indigo-700 transition">
                      Select Seat
                    </button>
                  </div>
                </div>
              );
            })}
          </div>
        </>
      )}
    </div>
  );
}