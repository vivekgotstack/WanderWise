...
export default function Trains() {
  ...
  const handleSearch = () => {
    ...
    recordActivity({
      type: "search",
      module: "trains",
      summary: `Searched trains ${source.trim().toUpperCase()} to ${destination.trim().toUpperCase()}`,
    });

    navigate(
      `/trains/results?source=${source.trim().toUpperCase()}&destination=${destination
        .trim()
  ...
  return (
    ...
    >
      ...
      <div className="w-full md:w-1/2 flex flex-col justify-center items-start p-6 md:p-12 lg:p-16 text-left">
        ...
        >
          Discover trains effortlessly. Check schedules, compare options, and
        ...
        <p className="mt-4 text-sm text-gray-500">
          Live schedules. Instant results.
        </p>
      </div>
    </div>
  );
